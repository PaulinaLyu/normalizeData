...
type ByIdType = { [key: string]: IPost };
type AllIdsType = string[];
...
const normalizeData = (unnormalizedData: IPost[]): IResultData => {
  const resultData: Partial<IResultData> = {};
  const byId: ByIdType = {};
  ...
  const allIds: AllIdsType = unnormalizedData.map((item) => {
    return item.id;
  });
  ...
  return resultData as IResultData;
};
